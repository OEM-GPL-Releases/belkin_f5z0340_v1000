...
LC_ALL:=C
LANG:=C
LIST_PACKAGE:=WeMo_Core DeviceControl AddlInfra RemoteAccess 
...
specific_clean:
	for packname in $(LIST_PACKAGE) $(WEMOAPP); do \
		if [ -d $(BUILD_DIR)/$$packname ]; then \
                	$(MAKE) package/$$packname/clean; \
        	fi; \
	done
