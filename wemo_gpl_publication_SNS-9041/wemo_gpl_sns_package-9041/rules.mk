...
WEMO_DEVICE_NAME:=$(shell echo $(BUILD_SUFFIX) | sed 's/_prod//')
ifeq ($(WEMO_DEVICE_NAME),insightCR)
  WEMOAPP:=WeMo_Insight
else
  WEMOAPP:=$(shell if [ -f .config ]; then grep -i CONFIG_PACKAGE_WeMo_$(WEMO_DEVICE_NAME) .config | sed 's/CONFIG_PACKAGE_//;s/=y//'; fi)
endif
...
DL_CACHE:=/wemo_dev_dl

DL_DIR:=$(if $(DL_DIR),$(DL_DIR),$(if $(call qstrip,$(CONFIG_DOWNLOAD_FOLDER)),$(call qstrip,$(CONFIG_DOWNLOAD_FOLDER)),$(if $(wildcard $(DL_CACHE)),$(DL_CACHE),$(TOPDIR)/dl)))
