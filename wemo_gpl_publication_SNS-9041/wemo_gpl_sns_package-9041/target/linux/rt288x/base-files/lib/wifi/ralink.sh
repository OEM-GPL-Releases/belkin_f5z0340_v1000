...
# We need to implement this "someday"
wlc() {
   true
}
...
#WNC0_tsunghao Mon 12 Aug 2013 09:56:20 AM CST, process interface "ra0"
        if [ "vif" = "cfg03485b" ];  then
#WNC0
		config_set "$vif" ifname "ra0${_c:+.$_c}"
#WNC0_tsunghao Mon 12 Aug 2013 09:56:20 AM CST
        fi
#WNC0
		_c=$((${_c:-0} + 1))
	done
...
#WNC0_tsunghao
                if [ "$device" = "apcli0" ]; then
                    for dev in apcli0; do
                        ifconfig "$dev" down 2>/dev/null >/dev/null && {
                                unbridge "$dev"
                        }
                        done
                else
                    for dev in ra0 ra0.1 ra0.2 ra0.3; do
                        ifconfig "$dev" down 2>/dev/null >/dev/null && {
                                unbridge "$dev"
                        }
                        done
