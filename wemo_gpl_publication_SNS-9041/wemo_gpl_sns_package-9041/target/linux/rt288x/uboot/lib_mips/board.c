...
#define RALINK_REG_PIO_SET				(RALINK_PRGIO_ADDR + 0x2C)
#define RALINK_REG_PIO_CLR				(RALINK_PRGIO_ADDR + 0x30)
#define RALINK_REG_PIO_SET_22_27		(RALINK_PRGIO_ADDR + 0x7C)
#define RALINK_REG_PIO_CLR_22_27		(RALINK_PRGIO_ADDR + 0x80)
...
void board_init_f(ulong bootflag)
{
	...
	*s-- = 0;
	...
	relocate_code (addr_sp, id, TEXT_BASE);	
#else
	debug ("relocate_code Pointer at: %08lx\n", addr);
...
void OperationSelect(void)
{
	...
	printf("   %d: System Load Linux Kernel then write to Flash via SERIAL. \n", SEL_LOAD_CRAMFS_WRITE_FLASH);
...
/*config bootdelay via environment parameter: bootdelay */
	...
	putc ('\n');
//#define GTK_KERN_B_ADDR	0xBC7C0000
	if(BootType == '3') {
		char *argv[2];
		//sprintf(addr_str, "0x%X", CFG_KERN_ADDR);
		sprintf(addr_str, "0x%X", OWRT_KERN_ADDR);
//printf("Abhishek addr_str=[%s]\n", addr_str);
		argv[1] = &addr_str[0];
		...
		case '2':
			...
			if (1) {
				...
				char *ProgramAdr = getenv("boot_A_adr");

				if(ProgramAdr == NULL) {
					program_address = CFG_KERN_ADDR-CFG_FLASH_BASE;
				}
				else {
					program_address = simple_strtoul(ProgramAdr, NULL, 16) - CFG_FLASH_BASE;
				}

				raspi_erase_write((u8 *)load_address, program_address, NetBootFileXferSize);
			}
#else //CFG_ENV_IS_IN_FLASH
		...
		case '6':
		     ...
		     * Date: March 19, 2013
		     *
		     * This code piece will download the FW image using SERIAL
		     */
...
void GPIO_Init(void)
{
	u32 OutBits = 0xfffabff;
	u32 SetBits = 0xf405f01;
	u32 ClearBits = ~0xf405f01;
	u32 Mode = 0x0040429c;
	u32 x;
	char *Var;
	int i;

	#define GPIO_0_21_MASK	0x3fffff
	#define GPIO_22_27_MASK	0x3f
	...
	{
		{"GPIO_Mode", &Mode},
		{"GPIO_Outputs", &OutBits},
		{"GPIO_Setbits", &SetBits},
		{"GPIO_Clrbits", &ClearBits},
		{NULL}
	};

	for(i = 0; EnvVars[i].Var != NULL; i++) {
		if((Var = getenv(EnvVars[i].Var)) == NULL) {
			printf("Warning: env variable %s not set, using default\n",
					 EnvVars[i].Var);
		}
		else {
			*EnvVars[i].Bits = simple_strtoul(Var, NULL, 16);
		}
	}

// Set direction for GPIO_0 ... GPIO_21
	*(volatile u32 *)(RALINK_REG_PIODIR) = cpu_to_le32(OutBits & GPIO_0_21_MASK);

// Set direction for GPIO_22 ... GPIO_27
	OutBits >>= 22;
	OutBits &= GPIO_22_27_MASK;

	*(volatile u32 *)(RALINK_REG_PIO5140DIR) = cpu_to_le32(OutBits);

// Set GPIO mode
	*(volatile u32 *)(RALINK_REG_GPIOMODE) = cpu_to_le32(Mode);

// Clear the clear bits
	*(volatile u32 *)(RALINK_REG_PIO_CLR) = cpu_to_le32(ClearBits & GPIO_0_21_MASK);
	ClearBits >>= 22;
	ClearBits &= GPIO_22_27_MASK;
	*(volatile u32 *)(RALINK_REG_PIO_CLR_22_27) = cpu_to_le32(ClearBits);

// Set the set bits
	*(volatile u32 *)(RALINK_REG_PIO_SET) = cpu_to_le32(SetBits & GPIO_0_21_MASK);
	SetBits >>= 22;
	SetBits &= GPIO_22_27_MASK;
	*(volatile u32 *)(RALINK_REG_PIO_SET_22_27) = cpu_to_le32(SetBits);

	x = le32_to_cpu(*(volatile u32 *)(RALINK_REG_PIODATA)) & GPIO_0_21_MASK;
	x |= le32_to_cpu(*(volatile u32 *)(RALINK_REG_PIO5140DATA)) << 21;
