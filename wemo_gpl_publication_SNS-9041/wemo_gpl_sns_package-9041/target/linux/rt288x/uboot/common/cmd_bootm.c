...
int Gemtek_boot_fail()
{
	printf("Boot image fail...\n");
	char *boot_state = getenv("bootstate");
//	char *check_a = getenv("check_a");
//	char *check_b = getenv("check_b");
	
	if(boot_state == NULL)
	{
		printf("bootstate ERROR!! Please check your uboot environment...\n");
	...
	else
	{
		...
		{
			printf("Reset to boot from B...\n");
			setenv("bootstate", "2");
			saveenv();
			//printf("Boot A addr_str=[%s]\n", argv[1]);
		}
		else if ( !strcmp(boot_state, "3") )
		{
			printf("Reset to boot from A...\n");
			setenv("bootstate", "0");
			saveenv();
			//sprintf(argv[1], "0x%X", GTK_KERN_B_ADDR);
			//printf("Boot B addr_str=[%s]\n", argv[1]);
		}
		//saveenv();
...
//printf("----> do_bootm()\n");
	...
	int	i, verify;
	char	*name, *s, *check_boot;
	int	(*appl)(int, char *[]);
	image_header_t *hdr = &header;
	char *BootAdr = "boot_A_adr";
	ulong DefaultAdr = OWRT_KERN_ADDR;
	//ulong DefaultAdr = CFG_KERN_ADDR;
...
//printf("----> do_bootm(), s=[%s]\n", s);
	...
	/*
	 * Abhishek: OpenWRT uses generic parition.
	 *  Only 1 bootsector exists. bootstate should always be 0
	 *  Explicitly set bootstate to 0
	 */
	setenv("bootstate", "0");
	saveenv();
#if 0
	...
	if(s == NULL)
	{
		printf("bootstate ERROR!! Reset bootstate to 0...\n");
		setenv("bootstate", "0");
		saveenv();
	}
#endif
	...
	else
	{
		...
		if ( !strcmp(s, "0") )
		{
			printf("Booting from A...\n");
		}
		else if ( !strcmp(s, "1") )
		{
			printf("Trying to boot from A...");
			check_boot = getenv("check_boot");
			if( check_boot == NULL || !strcmp(check_boot, "0") )
			{
				printf("\n");
				setenv("check_boot", "1");
				saveenv();
			...
			else
			{
				...
				saveenv();
				BootAdr = "boot_B_adr";
				DefaultAdr = GTK_KERN_B_ADDR;
			}
		}
		else if ( !strcmp(s, "2") )
		{
			printf("Booting from B...\n");
			BootAdr = "boot_B_adr";
			DefaultAdr = GTK_KERN_B_ADDR;
		}
		else if ( !strcmp(s, "3") )
		{
			printf("Trying to boot from B...");
			check_boot = getenv("check_boot");
			if( check_boot == NULL || !strcmp(check_boot, "0") )
			{
				printf("\n");
	...
	if (argc < 2) {
		...
		if((argv[1] = getenv(BootAdr)) == NULL) {
			printf("Warning: env variable '%s' not set.\n",BootAdr);
			sprintf(argv[1], "0x%X", DefaultAdr);
		    /*
		     * Abhishek: Setting the boot address if found NULL
		     */
			setenv(BootAdr, "bc030000");
			saveenv();
		}
	...
	case IH_COMP_BZIP2:
		...
		unsigned int destLen = 0;
                ...
                printf("Uncompression length is %d\n",destLen);
					 ...
					 flush_cache(ntohl(hdr->ih_load),destLen);
                break;
#endif /* CONFIG_LZMA */
	default:
		/*
